...
export function log({name, level, metadata, errorHandle}:ILogOption){
    return function(target:Object, propertyKey:string, descriptor:PropertyDescriptor){
        ...
        const actualLogger = Reflect.getMetadata("logDecoratorLogger", target.constructor)
        const actualLevel = level??Reflect.getMetadata("logDecoratorLevel",target.constructor)
        const preCompiledMetadata = Object.assign({},metadata, Reflect.getMetadata("logDecoratorMetaData", target.constructor))
        ...
        const actualErrorHandle = errorHandle??Reflect.getMetadata("logDecoratErrorHandler", target.constructor)
        descriptor.value = function(...args:any[]){
            try{
                ...
                actualErrorHandle(actualLogger,name,e as Error, actualMetadata)
            }
